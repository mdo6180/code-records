...
ssl_context = ssl.create_default_context(
    ...
    cafile=mkcert_ca,  # Used to validate client certs
)

ssl_context.load_cert_chain(
    certfile="certificate_root.pem",    # Your server cert (possibly with full chain)
    keyfile="private_root.key",         # Your server's private key
    password=None                       # Add password if the key is encrypted
)

ssl_context.verify_mode = ssl.CERT_REQUIRED  # Enforce client certificate validation
